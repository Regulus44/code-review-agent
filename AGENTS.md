...
阶段性开发更新完成后必须立即创建独立的 Git commit 作为 checkpoint；这里的“阶段性开发更新”包括代码、公共契约、测试、阶段计划、状态看板和开发日志的成组变更。提交前必须运行与改动范围匹配的检查，并在提交信息或提交说明中标明 Phase、变更范围和验证结果。未提交的阶段性更新不能被标记为完成，也不能作为进入下一阶段的依据；不同 Phase 的变更不得无说明地混入同一个提交。

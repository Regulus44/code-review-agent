  ...
  return [
    {
      ...
      execute: async (input, context) => { const value = await readFile(await new WorkspaceResolver(context.workspaceRoot).resolveExisting((input as { path: string }).path), "utf8"); return ok(value); },
    },
    {
      name: "glob", description: "List files under the workspace matching a simple glob.", inputSchema: object({ pattern: string }), executionMode: "parallel", riskLevel: "read", approvalMode: "auto", interruptBehavior: "cancel",
      execute: async (input, context) => ok(await globFiles(context.workspaceRoot, (input as { pattern: string }).pattern)),
    },
    {
    ...
    {
      name: "write_file", description: "Write UTF-8 text to a workspace file.", inputSchema: object({ path: string, content: string }), executionMode: "exclusive", riskLevel: "write", approvalMode: "ask", interruptBehavior: "cancel",
      execute: async (input, context) => { const args = input as { path: string; content: string }; const resolver = new WorkspaceResolver(context.workspaceRoot); const candidate = resolver.resolve(args.path); await mkdir(path.dirname(candidate), { recursive: true }); const target = await resolver.resolveForWrite(args.path); await writeFile(target, args.content, "utf8"); return ok({ path: args.path, bytes: Buffer.byteLength(args.content) }); },
    },
    {
      ...
      execute: async (_input, context) => runArgv("git", ["status", "--short", "--branch"], context.workspaceRoot, context.signal),
    },
    {
...
async function globFiles(root: string, pattern: string): Promise<string[]> { const normalized = pattern.replaceAll("\\", "/"); const regex = new RegExp("^" + normalized.split("*").map(escapeRegExp).join(".*") + "$", "u"); const output: string[] = []; await walk(root, root, async (file) => { const relative = path.relative(root, file).replaceAll("\\", "/"); if (regex.test(relative)) output.push(relative); }); return output.sort(); }
...
async function grepFiles(root: string, args: { pattern: string; path: string; maxResults: number }, signal: AbortSignal): Promise<string[]> { const resolver = new WorkspaceResolver(root); const base = await resolver.resolveExisting(args.path || "."); const info = await stat(base); const files: string[] = []; if (info.isFile()) files.push(base); else await walk(root, base, async (file) => { files.push(file); }); const regex = new RegExp(args.pattern, "u"); const results: string[] = []; for (const file of files) { if (signal.aborted) throw signal.reason ?? new Error("Cancelled"); const lines = (await readFile(file, "utf8")).split(/\r?\n/u); lines.forEach((line, index) => { if (results.length < args.maxResults && regex.test(line)) results.push(`${path.relative(root, file).replaceAll("\\", "/")}:${index + 1}:${line}`); }); if (results.length >= args.maxResults) break; } return results; }
function runArgv(command: string, args: string[], cwd: string, signal: AbortSignal): Promise<ToolResult> { return new Promise((resolve) => { const child = spawn(command, args, { cwd, shell: false, windowsHide: true }); let output = ""; let bytes = 0; let truncated = false; const append = (chunk: Buffer) => { bytes += chunk.byteLength; const currentBytes = Buffer.byteLength(output, "utf8"); if (currentBytes < MAX_PROCESS_OUTPUT_BYTES) output += chunk.toString("utf8").slice(0, MAX_PROCESS_OUTPUT_BYTES - currentBytes); if (bytes > MAX_PROCESS_OUTPUT_BYTES) truncated = true; }; child.stdout.on("data", append); child.stderr.on("data", append); const abort = () => child.kill(); signal.addEventListener("abort", abort, { once: true }); child.on("error", (error) => { signal.removeEventListener("abort", abort); resolve(fail("COMMAND_FAILED", error.message)); }); child.on("close", (code, signalName) => { signal.removeEventListener("abort", abort); const usage = { bytes, truncated }; if (signal.aborted || signalName) resolve({ ...fail("COMMAND_CANCELLED", "Command was cancelled"), output, usage }); else if (code === 0) resolve({ ...ok(output), usage }); else resolve({ ok: false, output, usage, error: { code: "COMMAND_EXITED", message: `Command exited with code ${code}` }, presentation: { kind: "terminal", title: "Command failed", text: output } }); }); }); }

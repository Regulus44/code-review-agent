...
describe("presentTodoPanel", () => {
  it("summarizes durable todo statuses and uses bounded details", () => {
    ...
    expect(view.summary).toContain("1/3 completed");
    expect(view.items[1]).toMatchObject({ statusLabel: "In progress", detail: "Editing" });
  });

  it("preserves an explicit empty state", () => {
    expect(presentTodoPanel([])).toMatchObject({ visible: false, total: 0, summary: "No todo items" });
  });
});

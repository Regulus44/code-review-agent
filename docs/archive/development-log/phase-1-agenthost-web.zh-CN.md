...
阶段计划：[phase-1-agenthost-web.zh-CN.md](../phases/phase-plans/phase-1-agenthost-web.zh-CN.md)
...
Phase 1 不再以 Web Shell 完成为退出条件，改以 [Agentic Coding Core 计划](../phases/phase-plans/phase-1-agentic-coding-core.zh-CN.md) 的 Phase 1A.0–1A.6 门禁为准。下一步先做 contracts → DeepSeek tool-call adapter → Agent Loop → P0 TypeScript 工具池 → permission resume，再完成真实 `read → edit → approve → test` smoke；Terminal、Plan/Todo、AskUser 已提升为 Phase 1A 的 P1，Subagent/A2A 暂不进入核心实现。
...
- 新增 [tool-migration-matrix.zh-CN.md](../../reference/tool-migration-matrix.zh-CN.md)，登记 P0/P1 工具与 DSH/Claude Code 行为参考、权限/工作区/模型视图和安全边界；
...
- 新增 [system-prompt-design.zh-CN.md](../../reference/system-prompt-design.zh-CN.md) 和 ADR-009，记录与 Claude Code section pipeline、DSH lifecycle/tool pipeline 的对应关系及当前明确不宣称的高级能力。

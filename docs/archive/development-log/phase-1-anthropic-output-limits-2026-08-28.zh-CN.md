...
按 [Token/Context/Tool 实施基线](../../reference/token-context-tool-limits-dsh-claude-code-implementation-plan.zh-CN.md) 完成 Anthropic-compatible 的双值输出解析：默认请求 `32000` tokens，协议硬上限 `64000` tokens，并保留模型级 `maxOutputTokens` 校验。本次不采用 DSH DeepSeek 的 `256000`。

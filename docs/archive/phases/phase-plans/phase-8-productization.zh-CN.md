...
- `docs/archive/phases/phase8-browser-evidence.json` 记录真实 Codex In-app Browser 的 600/900/1024 视口结果：无横向溢出、移动侧栏抽屉、Details 开关、Settings `aria-modal`/标题关联、Escape、focus restore 和可访问名称检查；
...
8.0.7 状态：`completed`（2026-08-25）。真实 Codex In-app Browser evidence 已覆盖计划列出的 9 组行为场景；`docs/archive/phases/phase8-browser-evidence.json` 和 `scripts/phase8-browser-evidence-gate.mjs` 对场景、fixture 参数、视口、ARIA/focus、Settings recovery 和六个视觉基线做可重复审计。8.0 关闭后，Phase 8 继续保留 8.3/8.5 的目标部署环境 smoke 作为 partial 边界。
...
- `docs/archive/phases/phase8-visual-baselines/manifest.json` 与 `scripts/phase8-visual-gate.mjs` 增加 600/900/1024 Sidebar 状态矩阵，保留六张真实 JPEG 基线。
...
- `docs/archive/phases/phase8-deployment-policy.json` 固定 schema supported range、upgrade-before-backup、migration lock、health/integrity/SSE readiness、retained rollback 和 runtime hardening；

...
评测已切换为简化的真实 Agent 会话流程：`run-agent-task.ts` 与 `run-pilot.ps1` 只负责隔离 workspace、启动 Agent、记录事件/日志/差异和汇总结果。独立 Grader、hidden patch、scope audit 及 Grader 自检入口已移除；后续验证按 [`../../coding-agent-simple-evaluation-plan.zh-CN.md`](../../evaluation/coding-agent-simple-evaluation-plan.zh-CN.md) 执行。下方早期阶段记录中的 Grader/step 结论仅作历史证据，不是当前运行要求。
...
- 代码与测试 checkpoint `61e3064`，文档 checkpoint `c5b747e`（基于 `d15a2ae` 收敛）；详细记录见 [阶段 6 集成门禁实施日志](../development-log/phase-6-integration-gate-2026-08-28.zh-CN.md)；
...
- 每个未启动调用都会从 `ToolRuntime.registry` 读取 live `executionMode`；exclusive 调用等待前置 parallel pool drain 后单独执行，后续 parallel 调用在 barrier 后继续；
...
- 上一阶段入口：实施基线中的阶段 5，按 DSH 默认最多 `10` 个并行工具调用实现统一 scheduler；该阶段已由上方 MR11 完成，并继续消费本阶段稳定的聚合预算与 replacement state。
...
- preview 在进入模型上下文和 `context/tool_result_persisted` receipt 前会对 `api_key`、`authorization`、`access_token`、`refresh_token`、`password`、`secret` 等 credential-shaped 字段脱敏；artifact 仍保留完整原始工具结果；
...
| Phase 3B Windows shell parity follow-up | completed（2026-08-28） | 阶段 1–6 已闭合：Windows 仅暴露 `pwsh`、POSIX 仅暴露 `bash`；PowerShell 路径解析、前后台 cwd/输出/取消/事件恢复、Agent schema/prompt 可见性和 `TOOL_NOT_FOUND` 合同通过；`pnpm typecheck`、`pnpm test`、`git diff --check` 通过 |
...
- 实现入口：[第五阶段浏览器验收实现记录](../development-log/dsh-session-replay-phase5-browser.zh-CN.md)、`apps/web/tests`、`scripts/phase5-browser-gate.mjs`；
...
- `docs/archive/phases/phase8-visual-baselines/manifest.json` 增加 600/900/1024 的 sidebar 状态矩阵（empty、long-list、search、workspace-menu、attention）；`scripts/phase8-visual-gate.mjs` 对稳定 shell markers/CSS 断言做可重复审计，既有六张 JPEG 基线保持不变。
...
- `docs/archive/phases/phase8-browser-evidence.json` 新增 8.0.7 真实浏览器场景证据：Goal/Plan/Question、Queue/Steer/Attachment、Workspace lifecycle、Tool/Permission/Diff/Job、Trajectory、Produced Files、Subagent interrupt/history、Settings/model failure、reconnect/replay/API restart，共 9 组场景。
...
- 独立 checkpoint：`1935dc5 fix(phase8): preserve conversation tool order`；详细根因、DSH 对照和回滚方式见 [ToolRow 对话顺序问题记录](../ui/ui-issue-tool-row-order.zh-CN.md)。
...
- 参考 DSH `packages/llm/llm/src/index.ts:LlmError`、`adapterFailureChunk()`、provider retry policy；Claude Code `src/services/api/withRetry.ts` 的 retry-after/backoff 与 `src/services/api/errors.ts` 分类。实施说明见 [P8.5-MR6 实施说明](../../reference/provider-model-routing-mr6-implementation.zh-CN.md)。验证：`pnpm typecheck`、LLM/Context/Runtime 定向测试、`git diff --check`。
...
- `docs/archive/phases/phase8-deployment-policy.json` 与 `scripts/phase8-upgrade-policy-gate.mjs` 审计 Docker non-root/read-only/no-new-privileges/cap-drop、bounded workspace 和 `upgrade=deferred-until-deployment-smoke`；`phase8-operations-gate` 已将 policy 与 v5 → v7 restore/rollback 联合验证。
...
## Phase 8.5 本地 host 凭据与可视化 Provider/Model 切换 slice（implemented，整体 8.5 仍未完成）
...
- [DSH Web 前端与 Agent 能力调研](../references/phase-7-dsh-web-research.zh-CN.md)
...
- M06 Session Memory Compact 已接入：Runtime 优先读取 host-owned `SessionMemoryStore`，按已摘要边界计算保留窗口，保护 tool pair/streaming response，并在边界不可靠或读取失败时记录失败 receipt 后回退 legacy compact；
...
- M14 Context Collapse 已完成能力边界登记：新增 host-backed `ContextCollapseCapability`，Runtime/API/Web 明确显示 `deferred`；read-time projection、background collapse、overflow drain、snip 均为 false。Claude Code 本地快照核心仍是 stub，因此不追加虚假 collapse 事件、不改变 M01–M13 的 model-view/compact/recovery/replay；Runtime/Web 定向测试与 `pnpm typecheck` 通过，完整算法等待真实 provider 场景证明前置模块不足后再单独立项。
...
- 新增 `docs/archive/phases/phase8-visual-baselines/manifest.json`、六个真实 JPEG 基线和 `scripts/phase8-visual-gate.mjs`；`pnpm test:phase8:visual` 已通过，校验 Shell/Settings 的 600/900/1024 尺寸、格式和 Plugins deferred 说明。
...
- 新增 `docs/archive/phases/phase8-browser-evidence.json`、`scripts/phase8-browser-evidence-gate.mjs` 与 `pnpm test:phase8:browser:evidence`；该 gate 固化真实 Codex In-app Browser 的视口、焦点、ARIA、Settings recovery 和视觉基线证据，不把静态 HTML marker 当作浏览器成功。
...
- 新增 [MCP 4B 契约审计](../../reference/mcp-4b-contract-audit.zh-CN.md)，冻结 DSH R0 对照、scope visibility、credential reference、generation 和 hostile fixture 矩阵；
...
实现与设计说明见 [../../system-prompt-design.zh-CN.md](../../reference/system-prompt-design.zh-CN.md)。
...
新增 [工具迁移矩阵](../../reference/tool-migration-matrix.zh-CN.md)，明确 DSH/Claude Code 行为参考、P0/P1 工具的 source/risk/execution/approval/workspace contract，以及行为 fixture 和安全回归索引。`packages/tools/src/behavior-fixtures.ts` 提供跨平台的 P0 contract fixture，新增 registry 对齐测试。

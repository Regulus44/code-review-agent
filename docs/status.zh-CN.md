...
6. RBAC、资源级 ACL、细粒度 quota、跨进程 Subagent、A2A 和 marketplace/远程插件生态尚未落地；S4 仅提供默认关闭的本地 bundle 最小运行时。
7. M1 的默认 Session Memory 使用 host-owned Markdown 文件，默认位于 SQLite 数据库同级、按数据库绝对路径哈希隔离的 `session-memory/<db-hash>/` 目录（可通过 `sessionMemoryRootDir` 覆盖）；M2/M3 的默认 Project Memory 使用同级 `project-memory/<db-hash>/<scopeKey>/`，支持 bounded `MEMORY.md`、topic frontmatter/references、显式 writer policy、原子写、stale 校验、manifest/词法召回和 last-good/incomplete scan；新增只读 `GET /v1/sessions/:id/memory` 与 Web Memory inspector。两类正文均不进入 EventStore/SSE；`sessionMemoryEnabled=false` 或 `projectMemoryEnabled=false` 可关闭并保留文件，恢复/损坏/写入失败均 fail closed。Memory 正文编辑 API 和 model-backed extractor 仍未落地，默认 Project Memory validator 也需由宿主显式注入。S0–S5 的 Skill contract/registry、SKILL.md loader、catalog/SkillTool、动态失效、paths 条件激活、本地 plugin bundle、gated MCP provider 与 `/v1/skills` 只读面已落地；默认 API Host 会注册 filesystem provider，但 `skillToolEnabled` 未显式设为 `true` 时 SkillTool、模型 catalog 和 `/name` 实际调用均不会启用。S3 watcher 默认关闭，`skillFilesystem.enabled=false` 可回滚；S5 MCP Skill provider 默认关闭，只有 `mcpSkills.enabled=true` 和显式 server allowlist 才发现远程 `skill://` 资源；不实现 marketplace、任意 HTTP 抓取或语义搜索。当前 Web 已提供 API/client、catalog presenter 和 Skill tool row，但 legacy `index.html` composer 尚未接入实际 suggestions 下拉组件。Skill 的 `userInvocable` 用户侧拒绝、`allowed-tools` 到 ToolRuntime 的实际收缩、真实 fork 子 Session 和 `modelToolExposed` capability 投影仍需补齐。

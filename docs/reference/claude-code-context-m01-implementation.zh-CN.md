...
本文件记录 M01 的实际实现，作为 `docs/reference/claude-code-context-management-research.zh-CN.md` 中 M01 规划的代码级落地补充。实现只参考 Claude Code 的结构和行为，没有复制 Claude Code 源码。
...
本节之后的 M01 原始实现记录保留当时的 16K/0 fallback 描述，用于追溯初始实现；当前运行时、API capability 和评测基线以本节、[Token/Context/Tool 实施基线](token-context-tool-limits-dsh-claude-code-implementation-plan.zh-CN.md)及 [阶段状态](../archive/phases/phase-status.zh-CN.md) 为准。

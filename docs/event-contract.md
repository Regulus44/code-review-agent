...
`skills/change` is a bounded invalidation lifecycle event. Its payload is `{ version: 1, revision, reason, provider?, scope? }`; S3 workspace mutations may additionally include `{ pathCount, paths }` where paths are at most 32 workspace-relative entries capped at 512 characters each. It never contains SKILL.md正文, absolute paths, prompt text, credentials or provider error details. Consumers refetch their own cwd/scope catalog and may safely ignore unknown reasons. A registry invalidation is not a successful Skill invocation and must not create a tool/result event. The M5 filesystem watcher is best-effort and emits the same provider invalidation at a debounced boundary; watcher failures keep the last-good candidate and mark the next provider observation incomplete until a bounded retry succeeds. Catalog invalidation paths are limited to `.claude/skills`, a Skill directory, or that directory's `SKILL.md`; changes below a Skill directory (for example `references/`, `scripts/`, or `assets/`) do not trigger catalog refresh and are observed by the next resource read.

Skill resource reads reuse `tool/call` / `tool/progress` / `tool/result`. The durable `tool/result` payload contains only Skill name, Skill-relative path, bounded offset/limit, size, digest, truncation/media metadata and an opaque `skill-resource` artifact receipt (`artifactId`, provider and availability metadata); it never contains resource正文, absolute provider paths, or presentation text containing正文. When `skillResourceArtifactReplay` is enabled, the receipt points to a host-owned immutable snapshot guarded by session/tenant ACL. Conversation replay reads that snapshot and reconstructs the model view; a missing snapshot produces `<skill_resource status="unavailable">` with bounded recovery metadata and never silently rereads the current workspace. Duplicate deferred commits are idempotent by `toolCallId`, and SSE exposes the same metadata-only event.

M6 的安全检查发生在每次资源读取而非 catalog 展示阶段：registry 先按 host-derived tenant scope 解析 winning provider，filesystem provider 再执行 Skill-relative lexical 校验、canonical realpath containment、regular-file/special-file gate、symlink policy、TOCTOU identity 复核和 bounded bytes/offset/line 检查。资源 symlink 默认拒绝；允许 contained symlink 时 canonical target 仍不得离开 Skill 目录。`read_skill_resource` 的 permission/risk contract 不受 permission preset 或 Skill frontmatter 提权；`scripts/` 只作为 UTF-8 文本读取，不产生执行事件。失败只写稳定错误 code 和最小 metadata，不能写入绝对路径、底层异常、资源正文或其它 tenant 的信息。

M7 回放与产品验收 fixture 覆盖 SkillTool → `read_skill_resource` 的多 step 顺序、资源窗口和资源变更边界；这些测试不扩展事件形状，继续要求 EventStore、JSON API 和 SSE 仅暴露 bounded metadata/receipt。

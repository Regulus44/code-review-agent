...
| Phase 7：DSH Web 前端收敛 | in_progress | typed Web API/SessionStore/Conversation projection/SSE replay、Tool presenter/lineage guard、permission/interaction surface、过期与重启恢复 render intent、Trajectory query/lane/inspector/timeline/fold/tail-follow、Task/Subagent/MCP details、非空 Delegation browser fixture、Trajectory history paging/prepend replay、1,250-record bounded fixture、Read-only/Edit/Test-Recovery Coding browser fixture 和 Settings/general/model/permission/capability surface 已接入主 Session 流；`pnpm typecheck`、`pnpm test`、Web 41 项测试、API 19 项测试、Storage 10 项测试、browser bundle 和真实 Read-only/Edit/Test-Recovery/Delegation/Trajectory/Settings replay smoke 通过；Deliverables、Shell 拆分、完整 capability surface 的剩余入口、统一五场景 browser gate、性能基线和 Phase 7.9–7.10 仍在推进 |
...
- 定向与全量验证：`pnpm typecheck`、`pnpm test`（全 workspace 通过）、`pnpm --filter @code-review-agent/web test`（41 tests）、`pnpm --filter @code-review-agent/tools test -- --run src/index.test.ts`（30 tests）、`pnpm --filter @code-review-agent/runtime test -- --run src/index.test.ts`（13 tests）、`pnpm --filter @code-review-agent/api test -- --run src/server.test.ts`（19 tests）、`pnpm --filter @code-review-agent/storage test -- --run src/index.test.ts`（10 tests）、`pnpm -F @code-review-agent/web run build:browser`、`git diff --check`；API/AgentHost recovery fixture、Delegation browser replay/cancel、child Session identity、Trajectory timeline/fold/tail-follow、load older/prepend、1,250-record search/bounded render、paused tail append 和 Settings dialog/escape smoke 均通过，browser console 无 warning/error。

下一切片：补齐 Deliverables/Produced Files 和 Settings 的错误、空态、键盘/窄屏细节，将 Read-only、Edit、Test/Recovery、Delegation、Inspection fixture 汇总为可重复的 Phase 7.10 browser gate，补齐长任务 terminal/job 输出与失败诊断，推进 Shell 拆分、Workspace/Session 导航和 1,000+ trajectory 多页性能基线；继续保持 EventStore replay、权限和 workspace 安全边界不变。

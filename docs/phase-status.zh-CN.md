...
| Phase 7：DSH Web 前端收敛 | in_progress | typed Web API/SessionStore/Conversation projection/SSE replay、Tool presenter/lineage guard、permission/interaction surface、过期与重启恢复 render intent、Trajectory query/lane/inspector/timeline/fold/tail-follow、Task/Subagent/MCP details、非空 Delegation browser fixture、Trajectory history paging/prepend replay、1,250-record bounded fixture、Read-only/Edit/Test-Recovery Coding browser fixture、Settings/general/model/permission/capability surface、modal keyboard/focus semantics、loading/error/reconnect banner、Deliverables/Produced Files render surface、workspace-scoped artifact API、统一五场景 browser/replay gate 和 typed Workspace→Session navigation projection 已接入主 Session 流；`pnpm test:phase7:browser` 通过五场景、artifact scope、重启恢复、幂等批准、1,250-record/2,501-event 分页回放，最近一次总耗时 2.18s、trajectory full replay 18.60ms；物理 Shell 拆分、Workspace/Session 生命周期 API、长任务 terminal/job 失败诊断和窄屏视觉基线仍在推进 |
...
下一切片：补齐各面板 loading/error/empty/reconnect 细节、长任务 terminal/job 输出与失败诊断，继续拆分物理 Shell、补 Workspace/Session 生命周期 API 和窄屏视觉基线；导航 projection 已落地，统一五场景 gate 继续作为验收基线。

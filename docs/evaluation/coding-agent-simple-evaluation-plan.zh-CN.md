...
`workspace-full-access` 与防污染 Prompt 的实现记录见 [`logs/workspace-full-access-evaluation-boundary-2026-08-30.zh-CN.md`](logs/workspace-full-access-evaluation-boundary-2026-08-30.zh-CN.md)。

命令与测试工具的应用层 workspace 防外溢方案见 [Workspace command guard 方案](../operations/workspace-command-guard-plan.zh-CN.md)。

评测 Prompt 固定模板见 `../../scripts/eval-mvp/evaluation-prompt.ts`。模板正文保持不变，每条任务只替换任务描述和当前 workspace 路径；Windows 进程统一以隐藏窗口方式执行。正式执行基线见 [`normal-web-agent-evaluation-baseline-2026-08-30.zh-CN.md`](normal-web-agent-evaluation-baseline-2026-08-30.zh-CN.md)。
...
准备干净 workspace
  ...
  → 发送任务描述
  → 通过 Web Agent 前端和同一会话事件流观察 Agent 自主读取、修改、运行和安装依赖
  → Agent 完成后保存对话、完整事件轨迹和代码差异
  → 通过会话事件记录复核证据完整性和 workspace 边界
...
如果机器进程完全失去响应，可以由评测人员手动停止，并记录为“运行中断”。这个操作是为了保护机器，不把它伪装成 Agent 的代码能力结论。不要为了查询状态而调用外部 shell 或反复启动桌面端 MCP 工具。
